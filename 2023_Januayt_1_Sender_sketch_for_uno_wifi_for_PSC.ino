...
void setup() {
  ...
  Wire.begin();
  BLE.scan(true); //added by copying https://github.com/arduino-libraries/ArduinoBLE/issues/202
}

void loop()
{   BLEDevice peripheral = BLE.available();
    if ((peripheral) and !gameOver)
    { Serial.print("Found ");
      Serial.println(peripheral.address());
      if ((peripheral.address() == "e5:97:ee:07:27:f2") or (peripheral.address() == "d5:1a:de:86:93:e9")) //should be all lower casse
      { BLE.stopScan();
        getsubscribedtoSensor(peripheral);
        BLE.scan(true);
      }
    }
}
...
/*{
  ...
  { 
     ...
     {
      if ( (cablePeripheral.address() == "E5:97:EE:07:27:F2")) //was >CABLE E5:97:EE:07:27:F2 bike is ==>>D5:1A:DE:86:93:E9 // ddeleted //D5:1A:DE:86:93:E9
      {
        BLE.stopScan();
...
void refreshSpeedandpower(BLEDevice cablePeripheral) // line 382
{
  if (speedCharacteristic.valueUpdated())
  {
      ...
      // I think that if stopped for two minutes game is over
